...
// NewReader receives an os.File type and returns a new reader
type NewReader func(*os.File) (io.Reader, error)

var formats []format

// RegisterFormat appends formats to a slice
...
func Open(file *os.File) (io.Reader, error) {
	...
	if found == nil {
		return nil, fmt.Errorf("open archive: can not determine format")
	}
